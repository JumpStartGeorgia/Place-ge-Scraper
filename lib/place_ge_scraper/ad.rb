...
class PlaceGeAd
  ...
  def scrape_breadcrums_info
    ...
    # now get the proptery sub type from the text at the end of the bread crumb
    # breadcrumb format: Home / For sale / Appartment / Kutaisi / FOR SALE, Apartments, 1 room, Kutaisi, I.chavchavadze #9b/35, urgently
    # end text format: text, text, text, text
    #  - sub type is the second set of text
    text = @page.css('div.breadcrums').text
    if !text.nil?
      # the text in the end might also contain a / so have to go through each item to see which has multiple ,s
      end_text = nil
      ...
      if !end_text.nil? && end_text.length > 0
        items = end_text.strip.split(',')
        if items.length > 1
          @property_sub_type = items[1].strip
        end
      end
    end
  end
  ...
  attr_accessor :property_sub_type
